...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startWifiRebootModal() {
    const modal = document.getElementById('wifiRebootModal');
    if (!modal) return;

    document.body.classList.add('wifi-modal-open');

    const titleEl = document.getElementById('wifiRebootTitle');
    const mainEl = document.getElementById('wifiRebootMain');
    const helpEl = document.getElementById('wifiRebootHelp');
    const countdownEl = document.getElementById('wifiRebootCountdown');
    const cancelBtn = document.getElementById('wifiRebootCancel');
    const cancelMsg = document.getElementById('wifiRebootCancelMsg');
    const cancelUrl = modal.dataset.cancelUrl || '/wifi/reboot/cancel';
    let remaining = parseInt(modal.dataset.countdown || '10', 10);
    let finished = false;
    ...
    const timer = window.setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        if (titleEl) titleEl.textContent = 'System wird neu gestartet';
        if (mainEl) mainEl.textContent = 'Bitte kurz warten.';
        if (helpEl) helpEl.textContent = 'Nach dem Neustart wieder mit „Autodartsinstall1“ verbinden.';
      }
    }, 1000);
    ...
    if (cancelBtn) {
      cancelBtn.addEventListener('click', async () => {
        ...
        cancelBtn.textContent = 'Bitte warten…';
        try {
          const r = await fetch(cancelUrl, { method: 'POST', cache: 'no-store' });
          const j = await r.json().catch(() => ({}));
          if (!r.ok || j.ok === false) {
            throw new Error(j.msg || 'Abbrechen fehlgeschlagen');
          }

          finished = true;
          window.clearInterval(timer);
          if (titleEl) titleEl.textContent = 'Neustart abgebrochen';
          if (mainEl) mainEl.textContent = 'Der WLAN-Kanal wird jetzt gewechselt.';
          if (helpEl) helpEl.textContent = 'Die Verbindung trennt sich gleich. Danach wieder mit „Autodartsinstall1“ verbinden.';
          if (cancelMsg) {
            cancelMsg.textContent = j.msg || 'Neustart abgebrochen. Bitte nach dem Kanalwechsel erneut verbinden.';
          ...
          if (cancelMsg) {
            cancelMsg.textContent = e && e.message
              ? e.message
              : 'Abbrechen konnte nicht bestätigt werden.';
            cancelMsg.style.display = 'block';
          }
        }
      });
